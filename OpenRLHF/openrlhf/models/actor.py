    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(pretrain_or_model, str):
            ...
            self.value_head = nn.Linear(hidden_size, 1, device=torch.cuda.current_device())
            # print(f"My Weight Std: {self.value_head.weight.std().item():.8f}")
            # quit()
            # self.value_head.to(torch_dtype)
            # LoRA
            if lora_rank > 0:
    ...
    def forward(
        ...
        if not return_action_log_probs and not return_logprobs:
            ...
            if allgather_logits and self.packing_samples:
                ...
                output['values'] = gather_and_pad_tensor(values, ring_attn_group, ring_attn_pad_len, indices, batch, seqlen)
            return output
        ...
        return (values, output) if return_output else values
