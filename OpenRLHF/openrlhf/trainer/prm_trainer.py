...
from openrlhf.models import PRMLoss, CRMLoss, TACRMLoss
...
class ProcessRewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        self.loss_fn = CRMLoss(self.placeholder_token_id, self.reward_token_ids, self.args.neg_weight)
        if self.args.mono_weight > 1e-8:
            print('TACRMLoss')
            self.loss_fn = TACRMLoss(self.placeholder_token_id, self.reward_token_ids, self.args.neg_weight,self.args.mono_weight)
        # Mixtral 8*7b
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                if self.aux_loss:
                    ...
                    aux_loss = 0
                prm_loss, loss_pos,loss_neg, acc, acc_pos, acc_neg = self.loss_fn(inputs, rewards, labels, return_acc=True)
                # prm_loss, acc = self.loss_fn(inputs, output.values, labels, return_acc=True)
                ...
                logs_dict = {
                    ...
                    "prm_loss_pos": loss_pos.item(),
                    "prm_loss_neg": loss_neg.item(),
                    "acc": acc.item(),
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                times += 1
                loss_sum += prm_loss.item()
                loss_pos_sum += loss_pos.item()
                loss_neg_sum += loss_neg.item()
                acc_sum += acc.item()
                acc_neg_sum += acc_neg.item()
                acc_pos_sum += acc_pos.item()
                bar_dict = {"eval prm_loss": loss_sum / times,"eval prm_loss_pos": loss_pos_sum / times,"eval prm_loss_neg": loss_neg_sum / times, "eval acc": acc_sum / times, "eval acc_pos": acc_pos_sum / times, "eval acc_neg": acc_neg_sum / times}
                step_bar.update()
