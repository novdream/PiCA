...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        if not self.config.trainer.val_only:
            ...
            # 执行分层随机采样
            self.val_dataset.dataframe = self.val_dataset.dataframe.groupby('data_source', group_keys=False).apply(
                lambda x: x.sample(n=min(len(x), samples_per_source), random_state=42)
        ...
        # 更新总样本量计数（可选）
        # max_val_samples = len(self.val_dataset.dataframe)
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                reward_tensor, answer_correct, em_score,  _ = self.val_reward_fn(test_batch)

                reward_tensor_lst.append(reward_tensor)
                answer_correct_lst.extend(answer_correct)
                em_score_lst.extend(em_score)
        ...
        for data_source, correct in data_source_score.items():  #评测指标从reward换成correct
        # for data_source, correct in data_source_score.items():
            metric_dict[f'val/test_score/{data_source}_f1'] = np.mean(correct)

        
        data_source_score = {}
        for i, correct in enumerate(em_score_lst):
            data_source = data_sources[i]
            if data_source not in data_source_score:
                data_source_score[data_source] = []
            data_source_score[data_source].append(correct)
        ...
        for data_source, correct in data_source_score.items():  #评测指标从reward换成correct
        # for data_source, correct in data_source_score.items():
            metric_dict[f'val/test_score/{data_source}_em'] = np.mean(correct)
            ...
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, answer_correct,em_score, step_scores = self.reward_fn(batch)
